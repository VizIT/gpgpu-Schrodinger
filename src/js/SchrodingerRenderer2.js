 ...
 */
function SchrodingerRenderer(gpgpUtility_, parent_, psiColor_, reColor_, imColor_, vColor_, xResolution_, yResolution_, E_, potential_, psiMax_, vMax_)
{
  "use strict";
  ...
  this.createProgram = function (gl)
  {
    ...
    fragmentShaderSource = "#ifdef GL_FRAGMENT_PRECISION_HIGH\n"
                         ...
                         + "        + color*(step(previousValue+scale2*halfDeltaY, value)*(step(previousValue, scale2*t-scale) - step(value, scale2*t-scale)))"
                         + "        + color*(step(value, previousValue+scale2*halfDeltaY)*(step(value, scale2*t-scale) - step(previousValue, scale2*t-scale)));"
                         + ""
                         + "  return pixel;"
                         + "}"
                         + ""
                         ...
                         + "  background = blend(background, color);"
                         + ""
                         + "  color = pixelColor(vColor, vMax, t, halfDeltaY, E, E);"
                         + "  background = blend(background, color);"
                         + ""
    ...
    EHandle            = gl.getUniformLocation(program, "E");
   ...
   * set uniforms.
   */
  this.setup = function(gpgpUtility, xResolution, yResolution, psiColor, reColor, imColor, E, v, vColor, psiMax, vMax)
  {
    var gl;
  ...
  this.setup(gpgpUtility, xResolution, yResolution, psiColor, reColor, imColor, E, potential, vColor, psiMax, vMax);
