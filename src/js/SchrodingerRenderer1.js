...
function SchrodingerRenderer(gpgpUtility_, parent_, psiColor_, reColor_, imColor_, vColor_, xResolution_, yResolution_, potential_, psiMax_, vMax_)
{
  ...
  this.createProgram = function (gl)
  {
    ...
    fragmentShaderSource = "#ifdef GL_FRAGMENT_PRECISION_HIGH\n"
                         ...
                         + ""
                         // Fill in discontinuities in the potential function
                         + "  color += vColor*(step(vPrevious+vMax2*halfDeltaY, v)*(step(vPrevious, vMax2*t-vMax) - step(v, vMax2*t-vMax)));"
                         + "  color += vColor*(step(v, vPrevious+vMax2*halfDeltaY)*(step(v, vMax2*t-vMax) - step(vPrevious, vMax2*t-vMax)));"
                         + "  background = blend(background, color);"
                         + ""
   ...
   * set uniforms.
   */
  this.setup = function(gpgpUtility, xResolution, yResolution, psiColor, reColor, imColor, v, vColor, psiMax, vMax)
  {
    var gl;
